...
class MMDetWandbHook(WandbLoggerHook):
    ...
    def after_train_epoch(self, runner):
        ...
        if (
            ...
            model_path = osp.join(
                self.ckpt_hook.out_dir, f"epoch_{runner.epoch + 1}.pth"
            )
    ...
    def _add_ground_truth(self, runner):
        ...
        for idx in self.eval_image_indexs:
            ...
            self.data_table.add_data(
                image_name,
                self.wandb.Image(
                    image, boxes=wandb_boxes, masks=wandb_masks, classes=self.class_set
                ),
            )
    ...
    def _get_wandb_masks(
        self, masks, labels, is_poly_mask=False, height=None, width=None
    ):
